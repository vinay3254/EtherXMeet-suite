...
export function useMeetingContract() {
  const { signer, account: walletAddress, provider: walletProvider } = useWallet();
  ...
  const getReadOnlyContract = useCallback(() => {
    if (!walletProvider) throw new Error('Provider not ready');
    return new ethers.Contract(contractData.address, contractData.abi, walletProvider);
  }, [walletProvider]);
  ...
  const pollEvents = useCallback(async (roomCode, fromBlock) => {
    if (!walletProvider) return { participants: [], messages: [] };
    try {
      ...
      const logs = await walletProvider.getLogs({
